			...
			foreach( $wpdb->get_results( $wpdb->prepare( "SELECT * FROM " . $wpdb->escape( $ecpt_db_tax_name ) . " WHERE id ='%d';", absint( $_GET['edit-tax'] ) ) ) as $key => $tax) {
				?>
					...
					<tr class="form-field">
						...
						<td>
							<input type="text" name="tax-singular" id="tax-singular" value="<?php echo esc_attr( utf8_decode($tax->singular_name) ); ?>" />
					...
					<tr class="form-field">
						...
						<td>
							<input type="text" name="tax-plural" id="tax-plural" value="<?php echo esc_attr( utf8_decode($tax->plural_name) ); ?>" />
