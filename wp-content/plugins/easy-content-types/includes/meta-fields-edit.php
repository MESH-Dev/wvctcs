...
<table class="wp-list-table widefat fixed posts">
	...
	<?php foreach( $wpdb->get_results( $wpdb->prepare("SELECT * FROM " . $wpdb->escape( $ecpt_db_meta_fields_name ) . " WHERE parent='%s' ORDER BY list_order;", $metabox->name ) ) as $key => $field) { ?>			
		<tr id="recordsArray_<?php echo absint( $field->id ); ?>" class="ecpt-field <?php if(ecpt_is_odd($i)) { echo 'alternate'; } ?>">
			...
			<td><?php echo esc_attr( stripslashes( utf8_decode($field->nicename) ) ); ?></td>
			<td>ecpt_<?php echo esc_attr( stripslashes( utf8_decode($field->name) ) ) ; ?></td>
			<td><?php echo $field->type; ?></td>
			<td><?php if(strlen($field->description) > 30) { echo esc_attr( substr(stripslashes(utf8_decode($field->description)), 0, 30) ) . '...'; } else { echo esc_attr( stripslashes(utf8_decode($field->description) ) ); } ?></td>
			<td>
